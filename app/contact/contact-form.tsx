            ...
            return (
              <div key={step.key} className="flex items-center">
                <div className="flex flex-col items-center">
                  ...
                  <span className={`text-[10px] sm:text-xs mt-1 max-w-[62px] sm:max-w-none text-center sm:whitespace-nowrap ${isActive ? 'text-primary-600 font-semibold' : isPast ? 'text-primary-600' : 'text-gray-500'}`}>
                    <T>{step.label}</T>
                  </span>
                </div>
                {index < steps.length - 1 && (
                  <div className={`hidden sm:block sm:w-16 h-0.5 sm:mx-2 ${isPast ? 'bg-primary-600' : 'bg-gray-300'}`} />
                )}
              </div>
            )
          })}
